...
| **AUC-13** | Detail page status and countdown | `type:feature` `priority:high` | AUC-11 | FR-DETAIL-07, 08 · TEAM M-16 | Status shown explicitly · live countdown updating at least once per second, **derived from the server-supplied end time** (RT-P3) · absolute end time shown in the viewer's local timezone with the zone explicit (NFR-USA-07) · **the end time CAN change** — BR-36 was amended on 2026-08-13 and an accepted bid in the final 15 s adds 30 s (capped at 20). The countdown must follow the server-supplied end time when it moves, and must not freeze on a value read once at mount (BR-36 as amended, RT-P3) |
...
| **BID-15** | **Automatic auction closing** | `type:feature` `area:bidding` `priority:critical` | **V-2**, BID-13 | FR-END-01→04 · US-15 · **SC-25, SC-26** · TEAM R-17 · **ARCH §15.3** | Marked Ended **within 30 seconds** of the end time (SC-25) · **automatic — no human action, no admin, no manual control** · **closes whether or not anyone is viewing** (SC-26) · **three triggers against one idempotent operation**: scheduled sweep (the SC-26 guarantee), on-read, on-bid-attempt (ARCH §15.3) · **closes against the CURRENT end time, never a cached or creation-time one** — BR-36 was amended on 2026-08-13 and the end time now moves (BR-36 as amended, SC-74) · **invocable directly in a test environment without waiting real time** (NFR-MNT-03) |
| **BID-15a** | **Anti-sniping end-time extension** *(added 2026-08-13 — BR-36 reversed)* | `type:feature` `area:bidding` `priority:critical` | BID-02, BID-15 | **BR-36 as amended** · **SC-74, SC-74a, SC-74b, SC-74c** · PRD Q7 | A bid **accepted** in the **final 15 seconds** moves `end_time` forward by **exactly 30 seconds** (SC-74) · **repeats** for each such bid (SC-74a) · **hard cap of 20 extensions**, after which the end time is final and a late bid is **still accepted** (SC-74b) · a **rejected** bid never extends (SC-74c) · the cap is a **`CHECK` constraint**, not a branch · `end_time` moves **forward only, in 30-second quanta, only inside `place_bid`**, and only together with `extension_count + 1` · atomic with the bid — both commit or neither · **shipped in the same migration as BID-15**, since closing and the extension are one mechanism seen from two sides |
...
**Must NOT be built here:** a bid increment (BR-32) · a maximum bid or price ceiling (BR-21) · a leading-bidder block (BR-24) · a reserve check or "reserve not met" outcome (BR-35) · manual or admin closing (PRD §4.3) · push notifications or any outbound message (PRD §16, RS-1) · any payment or contact step after the result (BR-34).
...
| **INT-08** | **Excluded-features audit** | **Mohammed** — `m7ya505` | `type:integration` `area:shared` `priority:high` | INT-03 | **SC-67** · PRD §19.0, SD-05 · TEAM §26 | Walk every screen and confirm **none** of the following was built: payment, checkout, wallet, card entry, refund, shipping, fulfillment · seller/winner messaging or contact exchange · cancel control or `Cancelled` state · edit screen or route · reserve field · bid increment · price ceiling · email-verification step · Admin role · ended auctions in the main listing · **the term "Demo Points"** · **any native mobile artifact** · SC-67 passes |
...
| 10 | The late bids **each push the end time 30 seconds further out** (BR-36 as amended 2026-08-13 — this step used to read "not extended by the late bids"). Once the bidding stops, or the 20-extension cap is reached, the auction reaches its end time and is marked Ended **within 30 seconds**, with nobody required to be watching | **BR-36 as amended**, FR-END-01→03, SC-25, SC-26, SC-74/74a/74b/74c |
...
| **10** | **A developer re-adds a deliberately removed check** — increment, ceiling, leading-bidder block, reserve, email verification. **Or, since 2026-08-13, the mirror image: a developer removes the anti-sniping extension** because a stale document still says the end time is fixed | Medium | ARCH §13.2a lists what the bid operation must **not** check · TEAM §26 lists what nobody may build · **`CLAUDE.md` §5 carries the BR-36 amendment and governs any document that still disagrees** · **INT-08 audits for it** · called out explicitly at code review |
