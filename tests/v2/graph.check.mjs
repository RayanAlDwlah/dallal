...
//      for a real reason, which is exactly what is wanted.
{
  ...
  // bound (archive line 500, index 499) and the no-update/no-delete comment
  // (archive line 485, index 484). Line numbers moved from 524/509 to 500/485
  // on 2026-08-16 because the archived copy is the snapshot taken at the V2
  // branch point, while main's copy gained a 24-line pause note ABOVE both
  // pins on 2026-08-15. Same file, same two statements, 24 lines earlier —
  // established by diffing the two copies, not by sliding the index until the
  // check went green.
  //
  // THESE TWO ASSERTIONS NO LONGER GUARD ANYTHING, AND THAT IS WRITTEN HERE
  // RATHER THAN LEFT TO BE DISCOVERED. supabase/archive-v1/ is frozen — V1 is
  // not developed on this branch — so a pin into it cannot fail whatever
  // anyone does to the product. They are kept because the crossings section
  // still argues from V1's starting position and a reader has to be able to
  // check that argument. They DOCUMENT; they do not DEFEND.
  //
