...
#
# EMPTY SINCE 2026-08-16, AND EMPTY IS THE HEALTHY STATE HERE.
#
# It held three entries, all excused for one root cause: they needed
# credentials to a real Supabase project, and this repository is public
# (CLAUDE.md §6). All three — tests/auth/identity-e2e.check.mjs,
# tests/auth/session.check.mjs, tests/realtime/reconnect.check.mjs — were
# deleted with the V1 product on this branch, and this script reported all
# three as STALE, which is the stale-entry check doing exactly its job.
#
# Do not read an empty allowlist as "nothing is excused, so nothing is
# missing." It means every suite that EXISTS is run. The suites that do not
# exist are a different question, and the largest of them — a V2 database suite
# for `place_bid` — is written up in the header of .github/workflows/ci.yml
# rather than here, because an allowlist can only name files it can stat.
# ---------------------------------------------------------------------------
ALLOWLIST="
"
