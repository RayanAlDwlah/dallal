...
| **R3** | [D-03](D-03-sessions.md) — sessions: a room of lots with a start time, and pause moves `end_time` forward | `PRD.md:784` **BR-16**: *"Nobody may change an auction's end time — not the seller, not an admin, not any edit path… **The single exception** is the automatic anti-sniping extension in BR-36"*. **And §19.2, which no record cited until 2026-08-15**: `:1847` excludes **Multiple quantity / lots** — *"A fundamentally different auction model"* — and `:1846` excludes **Scheduled future start times** — *"Adds a third lifecycle state with no MVP demand"* | **YES — and it is live on `main` today** | BR-16 must go from *the single exception* to two named doors, **and** §19.2 must un-mark lots and scheduled starts the way it un-marked anti-sniping for `BR-36`. How far that reaches on the board is open — see the third blockquote in [`../v2/TICKETS.md`](../v2/TICKETS.md) |
